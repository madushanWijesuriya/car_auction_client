...
<template>
    <div class="flex items-center min-h-screen p-4 bg-gray-100 lg:justify-center">
        <div
            ...
            <div class="ml-3 mt-3 text-2xl text-blue-900 font-bold tracking-wider text-center">
                <a href="#">Log In</a>
            </div>
            <div
                ...
                <form class="flex flex-col space-y-5">
                    <div class="flex flex-col space-y-1">
                        ...
                        <input type="email" id="email" autofocus
                            ...
                            :class="errorList.some(e => e.email) ? 'border-red-800' : 'border-gray-300'"
                            v-model="form.email"    
                        />
                        <div class="error-list">
                            <div :class="['error text-red-900 font-bold', idx !== 0 ? 'mb-1' : '']" v-for="(error, idx) in errorList.filter(e => e.email)" :key="idx">{{ error.message }}</div>
                        </div>
                    </div>
                    <div class="flex flex-col space-y-1">
                        ...
                        <input type="password" id="password"
                            ...
                            :class="errorList.some(e => e.password) ? 'border-red-800' : 'border-gray-300'"
                        />
                        <div class="error-list">
                            <div :class="['error text-red-900 font-bold', idx !== 0 ? 'mb-1' : '']" v-for="(error, idx) in errorList.filter(e => e.password)" :key="idx">{{ error.message }}</div>
                        </div>
                        <div class="flex items-center justify-end mt-4 mb-4">
                            <a href="#" class="text-sm text-blue-900 hover:underline focus:text-blue-600">Forgot
                                Password?</a>
                        </div>
