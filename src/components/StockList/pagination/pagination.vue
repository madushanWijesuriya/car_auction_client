<script setup>
import { toRefs, computed } from 'vue'
const props = defineProps(['indexingDetails'])
const { indexingDetails } = toRefs(props)
const emit = defineEmits(['go-to-first-page', 'go-to-last-page', 'change-page'])

let paginationList = computed(() => {
  const lastPageNo = indexingDetails.value.lastPage
  let array = []
  if (lastPageNo > 4) {
    for (let i = lastPageNo; i > lastPageNo - 4; i--) {
      array.push(i)
    }
  } else {
    for (let i = lastPageNo; i > 0; i--) {
      array.push(i)
    }
  }
  array = array.sort((a, b) => a - b)
  return array
})

const goToFirstPage = () => {
  emit('go-to-first-page')
}

const goToLastPage = () => {
  emit('go-to-last-page')
}

const changePage = (pageId) => {
  emit('change-page', pageId)
}
</script>
<template>
  <div
    class="w-full my-3 space-y-5 lg:space-y-0 lg:flex text-gray-600 lg:gap-3 justify-center items-center"
  >
    <div class="flex w-full justify-between text-[#08246C] font-semibold">
      <div>
        Showing {{ indexingDetails.from }} to {{ indexingDetails.to }} of
        {{ indexingDetails.total }} entries
      </div>
      <div class="flex gap-2">
        <div>Show</div>
        <div class="px-4 rounded-md border border-black">
          {{ indexingDetails.perPage }}
        </div>
        <div>Entries</div>
      </div>
    </div>
    <div class="flex gap-2 justify-center">
      <el-tooltip
        class="box-item"
        effect="dark"
        content="Go to first"
        placement="top-start"
      >
        <div class="flex justify-center items-center">
          <svg
            class="cursor-pointer"
            width="7"
            height="15"
            viewBox="0 0 7 15"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            @click="goToFirstPage"
          >
            <path
              d="M5.83749 14.0013C5.68809 14.0018 5.54048 13.9688 5.4055 13.9048C5.27052 13.8407 5.15161 13.7473 5.05749 13.6313L0.227488 7.63125C0.0804062 7.45232 0 7.22788 0 6.99625C0 6.76463 0.0804062 6.54018 0.227488 6.36125L5.22749 0.361252C5.39723 0.157036 5.64114 0.0286112 5.90556 0.0042315C6.16999 -0.0201482 6.43327 0.0615137 6.63749 0.231252C6.8417 0.400991 6.97013 0.644902 6.99451 0.909329C7.01889 1.17375 6.93723 1.43704 6.76749 1.64125L2.29749 7.00125L6.61749 12.3613C6.73977 12.508 6.81745 12.6868 6.84133 12.8763C6.86521 13.0659 6.83429 13.2583 6.75223 13.4308C6.67018 13.6034 6.54042 13.7488 6.37831 13.8499C6.2162 13.9509 6.02852 14.0035 5.83749 14.0013Z"
              fill="black"
            />
          </svg>
        </div>
      </el-tooltip>
      <div
        v-for="(pageNo, index) in paginationList"
        :key="index"
        :class="[
          'px-3 rounded-md py-1 text-black border border-black cursor-pointer',
          Number(indexingDetails.currentPage) === Number(pageNo)
            ? 'bg-[#08246C] text-teal-50'
            : 'bg-[#E6E7E8]',
        ]"
        @click="changePage(pageNo)"
      >
        {{ pageNo }}
      </div>
      <el-tooltip
        class="box-item"
        effect="dark"
        content="Go to last"
        placement="top-start"
      >
        <div class="flex justify-center items-center">
          <svg
            class="cursor-pointer"
            width="7"
            height="15"
            viewBox="0 0 7 15"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            @click="goToLastPage"
          >
            <path
              d="M1.16251 0.998748C1.31191 0.99824 1.45952 1.03121 1.5945 1.09524C1.72948 1.15927 1.84839 1.25273 1.94251 1.36875L6.77251 7.36875C6.91959 7.54768 7 7.77212 7 8.00375C7 8.23537 6.91959 8.45982 6.77251 8.63875L1.77251 14.6387C1.60277 14.843 1.35886 14.9714 1.09444 14.9958C0.83001 15.0201 0.566729 14.9385 0.362512 14.7687C0.158296 14.599 0.0298715 14.3551 0.00549173 14.0907C-0.018888 13.8262 0.0627732 13.563 0.232512 13.3587L4.70251 7.99875L0.382513 2.63875C0.26023 2.49196 0.182552 2.31322 0.158673 2.12367C0.134793 1.93412 0.16571 1.7417 0.247766 1.56917C0.329822 1.39665 0.459582 1.25123 0.621692 1.15014C0.783803 1.04905 0.971478 0.996516 1.16251 0.998748Z"
              fill="#08246C"
            />
          </svg>
        </div>
      </el-tooltip>
    </div>
  </div>
</template>
