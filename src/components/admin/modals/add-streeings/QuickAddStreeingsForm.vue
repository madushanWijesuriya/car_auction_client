...
const submitForm = async () => {
  try {
    ...
    const response = await httpResource.post(
      '/api/staff/vehicle/streeings/quickAdd',
      {
        name: form?.name,
      }
    )
        ...
        <div class="add-car">
          <SectionMain>
            <CardBox form @submit.prevent="submit">
              <SectionTitleLineWithButton :icon="mdiCarEstate" title="Add streeing Model" main>
              </SectionTitleLineWithButton>
              <FormField label="streeing" help="">
                <FormControl v-model="form.name" type="text" placeholder="streeing" />
              </FormField>
            </CardBox>
          </SectionMain>
          <SectionMain>
