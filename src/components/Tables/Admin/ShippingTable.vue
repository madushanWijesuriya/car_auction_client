...
import AdminEditShippingModal from '@/components/admin/modals/AdminEditShippingModal.vue'
...
let shipping = ref(null)
const toast = useToast()
const openEditModel = async (vehicleId) => {
  if (items.value && items.value.length > 0) {
    shipping.value = items.value.find((v) => v.id === vehicleId)
  }
  await nextTick()
